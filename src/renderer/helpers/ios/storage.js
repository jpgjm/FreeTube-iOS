...
// iOS 版ストレージヘルパ。
//
// Android 版と同じ仮想 URI スキーム `data://` を使う:
//   - `data://` はアプリ Documents 直下 (Swift 側で `FileManager` の
//     `.documentDirectory` にマップ)
//   - `data://profiles.db` などが Documents/profiles.db に対応する
//   - Android の SAF (Storage Access Framework) 相当は iOS では
//     UIDocumentPickerViewController でユーザ選択ディレクトリの
//     ブックマークを永続化することで実現する。ブックマーク URI は
//     `bookmark://<uuid>` の形で Swift 側で管理される。
//
// このモジュール自体は Android 版と 100% 同じ挙動になるよう記述してある。
// -----------------------------------------------------------------------------
...
/**
 * @typedef {Array<IOSFile>} FileList
*/
